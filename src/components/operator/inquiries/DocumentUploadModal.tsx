...
import { FiUpload, FiX, FiFileText, FiAlertCircle } from 'react-icons/fi';
...
export function DocumentUploadModal({
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const documentResponse = await fetch(`/api/operator/inquiries/${inquiryId}/documents`, {
        method: 'POST',
  ...
  const handleClose = () => {
    setFile(null);
    setDocumentType('APPLICATION_FORM');
    setDescription('');
    setUploadProgress(0);
    setError('');
    onClose();
