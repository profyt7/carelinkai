...
export function AssignStaffModal({ isOpen, onClose, inquiryId, inquiryData }: AssignStaffModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/operator/inquiries/${inquiryId}/assign`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          assignedTo: formData.staffMemberName,
          internalNotes: formData.notes ? `Assigned to ${formData.staffMemberName}: ${formData.notes}` : `Assigned to ${formData.staffMemberName}`,
        }),
      });
