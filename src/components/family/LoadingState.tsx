      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {[...Array(count)].map((_, i) => (
          <div key={i} className="rounded-lg border bg-white p-6 shadow-sm animate-pulse">
            <div className="h-4 w-3/4 bg-neutral-200 rounded mb-3"></div>
            <div className="h-3 w-1/2 bg-neutral-200 rounded mb-3"></div>
            <div className="h-3 w-1/3 bg-neutral-200 rounded mb-4"></div>
            <div className="flex gap-2 mb-4">
              {[...Array(2)].map((_, j) => (
                <div key={j} className="h-3 w-12 bg-neutral-200 rounded"></div>
              ))}
            </div>
            <div className="mt-4 flex items-center">
              <div className="mr-2 h-8 w-8 rounded-full bg-neutral-200"></div>
