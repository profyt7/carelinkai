    ...
    <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="lg:col-span-2">
            ...
            <input
              ...
              className="w-full border border-neutral-300 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-neutral-700 mb-2 flex items-center gap-2">
      ...
      {docs.length === 0 ? (
        <div className="bg-white rounded-lg border border-neutral-200 p-12 text-center">
          ...
          <p className="text-neutral-500">No documents found matching your filters</p>
        </div>
      ) : (
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {docs.map((doc) => (
            <div
              key={doc.id}
              className="group flex flex-col rounded-xl border border-neutral-200 bg-white p-6 shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1 hover:border-primary-300"
            >
              {/* Icon Header */}
              <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-lg bg-gradient-to-br from-primary-500 to-cyan-400 shadow-lg group-hover:shadow-xl transition-all duration-300">
                <FileText className="h-6 w-6 text-white" />
              </div>
              
              {/* Title */}
              <h3 className="mb-2 line-clamp-2 font-semibold text-neutral-900 text-lg group-hover:text-primary-600 transition-colors duration-200">
                {doc.title}
              </h3>
              
              {/* Type Badge */}
              <span className="mb-3 inline-block w-fit rounded-full bg-gradient-to-r from-primary-100 to-cyan-50 px-3 py-1 text-xs font-semibold text-primary-800 border border-primary-200">
                {doc.type.replace(/_/g, ' ')}
              </span>
              
              {/* Meta Info */}
              <div className="mb-3 text-xs text-neutral-500 flex items-center gap-2 font-medium">
                <span>{new Date(doc.createdAt).toLocaleDateString()}</span>
                <span className="w-1 h-1 rounded-full bg-neutral-400"></span>
                <span className="font-semibold text-neutral-700">{formatBytes(doc.fileSize)}</span>
              </div>
              ...
              <div className="mb-4 flex flex-wrap gap-1.5">
                {doc.tags.slice(0, 3).map((tag) => (
                  <span
                    key={tag}
                    className="rounded-full bg-gradient-to-r from-gray-100 to-gray-50 px-2.5 py-1 text-[10px] font-semibold text-neutral-700 border border-neutral-200 hover:border-neutral-300 transition-colors duration-200"
                  >
                    #{tag}
                  </span>
                ))}
              ...
              <div className="mt-auto flex items-center justify-between pt-4 border-t border-neutral-100">
                <div className="flex items-center gap-2">
                  <div className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-primary-500 to-cyan-400 text-xs font-bold text-white shadow-md">
                ...
                <a
                  ...
                  className="inline-flex items-center gap-1.5 rounded-lg bg-gradient-to-r from-primary-600 to-cyan-500 px-3 py-1.5 text-xs font-semibold text-white hover:from-primary-700 hover:to-cyan-600 hover:shadow-lg transition-all duration-200 transform hover:scale-105"
                >
