...
interface NoteEditModalProps {
  ...
  onClose: () => void;
  familyId: string;
  note: { id: string; title: string; content: any; tags?: string[] };
...
export default function NoteEditModal({
  ...
  // Initialize form when note changes or modal opens
  useEffect(() => {
    if (isOpen && note) {
      setTitle(note.title || '');
      setContent(note.content?.content || note.content?.plainText || '');
      setTags(note.tags || []);
      setTagsInput('');
      setError(null);
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (process.env.NODE_ENV !== 'production') {
          ...
          console.error('[NoteEditModal] update note failed:', errorData);
        }
        throw new Error(
          errorData.details || errorData.error || 'Failed to update note'
        );
      }
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto">
      ...
      <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
        <div className="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
          ...
          {/* Header */}
          <div className="mb-4">
            <h3 className="text-lg font-medium leading-6 text-gray-900">Edit Note</h3>
