...
// Deterministic hash-based embedding fallback (dim 128). Used when no embedding provider is configured.
// Resident matching still works — it falls back to structured scoring only.
...
export function summarizeHome(home: Record<string, any>): string {
  ...
  if (home["amenities"]?.length) {
    const am = Array.isArray(home["amenities"]) ? home["amenities"] : [];
