    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            ...
            disabled={loading}
            className="inline-flex items-center gap-2 rounded-md border border-neutral-300 bg-white px-3 py-2 text-sm hover:bg-neutral-50 disabled:opacity-50"
          >
      ...
      {loading ? (
        <div className="py-16 text-center">
          ...
          <p className="mt-4 text-neutral-600">Loading favorites...</p>
        </div>
      ) : error ? (
        <div className="rounded-lg border border-error-200 bg-error-50 p-8 text-center">
          <p className="text-error-600 mb-4">{error}</p>
          <button
            onClick={loadFavorites}
            className="inline-flex items-center gap-2 rounded-md bg-error-600 px-4 py-2 text-sm text-white hover:bg-error-700"
          >
            <FiRefreshCw size={16} />
            Try Again
          </button>
        ...
        <div className="py-16 text-center">
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            <Link href="/marketplace" className="inline-flex items-center gap-2 rounded-md border border-neutral-300 bg-white px-4 py-2 text-sm hover:bg-neutral-50">
              <FiUsers size={16} />
              Marketplace
            </Link>
          </div>
    ...
    return (
      <div className="relative group bg-white border rounded-lg overflow-hidden hover:shadow-md transition-shadow">
        ...
        <Link href={`/homes/${home.id}`} className="block">
          ...
          <div className="p-4">
            <h3 className="font-semibold text-neutral-900 mb-1">{home.name}</h3>
            {home.address && (
            ...
            <p className="text-sm text-neutral-600 line-clamp-2">{home.description}</p>
          </div>
        </Link>
      </div>
    );
        ...
        <Link href={`/marketplace/caregivers/${cg.id}`} className="block">
          <div className="flex items-start mb-3">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-neutral-900 truncate">{cg.name}</h3>
              {cg.yearsExperience && (
                <p className="text-sm text-neutral-600">{cg.yearsExperience} years experience</p>
              )}
              {cg.ratingAverage && (
                <div className="flex items-center text-sm text-neutral-600 mt-1">
                  <FiStar size={14} className="text-warning-400 fill-current mr-1" />
          ...
          {cg.bio && (
            <p className="text-sm text-neutral-600 line-clamp-2">{cg.bio}</p>
          )}
        </Link>
      </div>
    );
          ...
          <div className="flex items-start mb-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <h3 className="font-semibold text-neutral-900 truncate">{provider.businessName}</h3>
                {provider.isVerified && (
              ...
              <p className="text-sm text-neutral-600">{provider.contactName}</p>
              {provider.yearsInBusiness && (
                <p className="text-sm text-neutral-600">{provider.yearsInBusiness} years in business</p>
              )}
            </div>
          </div>
          {provider.bio && (
            <p className="text-sm text-neutral-600 line-clamp-2 mb-2">{provider.bio}</p>
          )}
          {provider.serviceTypes.length > 0 && (
            <div className="flex flex-wrap gap-1">
              {provider.serviceTypes.slice(0, 3).map((service) => (
                <span key={service} className="inline-flex items-center rounded-full bg-neutral-100 px-2 py-0.5 text-xs text-neutral-700">
                  {service}
                </span>
              ))}
      ...
      <div className="relative group bg-white border rounded-lg p-4 hover:shadow-md transition-shadow">
        ...
        {listing.status && (
          <span className={`absolute left-2 top-2 inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${
            listing.status === 'OPEN' ? 'bg-success-100 text-success-800' : 
            listing.status === 'HIRED' ? 'bg-primary-100 text-primary-800' : 
        ...
        <Link href={`/marketplace/listings/${listing.id}`} className="block pt-6">
          <h3 className="font-semibold text-neutral-900 mb-1">{listing.title}</h3>
