        ...
        <div className="flex flex-col md:flex-row md:space-x-6">
          ...
          <div className="flex-1">
            ...
            {!isLoading && viewType === "map" && searchResults.length > 0 && (
              <div 
                ref={mapRef} 
                className="relative h-[calc(100vh-12rem)] w-full overflow-hidden rounded-lg border border-neutral-200 bg-neutral-100"
              >
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="mt-2 text-neutral-600">
                      Map view would display {searchResults.length} homes
                    </p>
                ...
                {/* Map markers - would be actual markers in production */}
                <div className="absolute inset-0">
                  {searchResults.map((home, index) => (
                    <div
                      ...
                      className="absolute"
                      style={{
                        // Position markers in different locations for demo
                        left: `${10 + (index * 15) % 80}%`,
                        top: `${15 + (index * 12) % 70}%`,
                      }}
                    >
                      <div className="relative cursor-pointer">
                        <div className="flex h-8 w-8 items-center justify-center rounded-full bg-primary-500 text-white shadow-md">
                          ${Math.floor((home.priceRange.min || 3000) / 1000)}k+
                        </div>
                        {/* Tooltip on hover */}
                        <div className="absolute left-1/2 top-full z-10 mt-1 hidden -translate-x-1/2 transform rounded-md bg-white p-2 shadow-lg group-hover:block">
                          <p className="whitespace-nowrap text-xs font-medium">{home.name}</p>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                {/* Mini list view when in map mode */}
                <div className="absolute bottom-4 left-4 right-4 flex space-x-4 overflow-x-auto pb-4">
                  {searchResults.slice(0, 3).map((home) => (
                    <div
                      key={`mini-${home.id}`}
                      className="h-32 w-64 shrink-0 overflow-hidden rounded-lg bg-white shadow-md"
                    >
                      <Link href={`/homes/${home.id}`} className="flex h-full">
                        <div className="relative h-full w-1/3">
                          {home.imageUrl ? (
                            ...
                            <div className="flex h-full w-full items-center justify-center bg-neutral-200">
                              <FiHome className="h-6 w-6 text-neutral-400" />
                            </div>
                          )}
                        </div>
                        <div className="flex w-2/3 flex-col p-2">
                          <h4 className="mb-1 line-clamp-1 text-sm font-medium">{home.name}</h4>
