...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const messageId = Sentry.captureMessage('🧪 Edge runtime Sentry test message', 'info');
    ...
    const testError = new Error('🧪 Edge runtime Sentry test error - This is intentional for testing');
    ...
    Sentry.addBreadcrumb({
      category: 'test',
      message: 'Edge runtime test breadcrumb',
      level: 'info',
    });
    ...
    Sentry.withScope((scope) => {
      ...
      Sentry.captureMessage('🧪 Edge runtime test with context', 'warning');
    });
