 ...
 * GET /api/shifts/my
 * 
 * Lists shifts assigned to the authenticated caregiver
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Query shifts assigned to this caregiver with pagination
    const [shifts, total] = await Promise.all([
      prisma.caregiverShift.findMany({
        where: { 
          caregiverId: caregiver.id
        },
        orderBy: { startTime: "desc" }, // Most recent shifts first
        skip,
        take: limit,
        include: {
          home: {
    ...
    console.error("[SHIFTS API] Error fetching caregiver shifts:", error);
    
    // Provide more specific error messages
    let errorMessage = "Failed to fetch your shifts";
