    ...
    if (useCloudinary) {
      ...
      const uploadResult = await new Promise<any>((resolve, reject) => {
        cloudinary.uploader
          .upload_stream(
            {
              ...UPLOAD_PRESETS.FAMILY_DOCUMENTS,
              folder: `${UPLOAD_PRESETS.FAMILY_DOCUMENTS.folder}/${metadata.familyId}`,
            },
            (error, result) => {
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Delete underlying file (support Cloudinary, S3, and legacy FS path)
    // Try to get metadata for Cloudinary public ID
    const doc = await prisma.familyDocument.findUnique({
      where: { id: documentId },
      select: { metadata: true, fileUrl: true }
    });
