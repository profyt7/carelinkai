...
export default function AdminCaregiversPage() {
  ...
  const getBackgroundCheckBadge = (status: string) => {
    const badges: Record<string, { color: string; icon: React.ReactNode; label: string }> = {
      NOT_STARTED: { color: 'bg-neutral-100 text-neutral-800', icon: <FiClock size={14} />, label: 'Not Started' },
      PENDING: { color: 'bg-warning-100 text-warning-800', icon: <FiClock size={14} />, label: 'Pending' },
      CLEAR: { color: 'bg-success-100 text-success-800', icon: <FiCheck size={14} />, label: 'Clear' },
      CONSIDER: { color: 'bg-warning-100 text-warning-800', icon: <FiAlertTriangle size={14} />, label: 'Consider' },
      EXPIRED: { color: 'bg-error-100 text-error-800', icon: <FiX size={14} />, label: 'Expired' },
    };
  ...
  const getEmploymentStatusBadge = (status?: string) => {
    ...
    return (
      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${colors[status] || 'bg-neutral-100 text-neutral-800'}`}>
        {status.replace('_', ' ')}
      </span>
    );
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-neutral-900">Caregiver Management</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-lg shadow p-4">
          <div className="text-sm text-neutral-600">Total Caregivers</div>
        ...
        <div className="bg-white rounded-lg shadow p-4">
          <div className="text-sm text-neutral-600">Pending Verification</div>
      ...
      <div className="bg-white rounded-lg shadow mb-6 p-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-neutral-700 mb-1">Background Check</label>
            <select
        ...
        <div className="mt-4 flex justify-end">
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-neutral-100 text-neutral-700 rounded-md hover:bg-neutral-200"
          >
            <FiDownload size={16} />
            Export CSV
          </button>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        {loading ? (
          <div className="p-8 text-center">
            ...
            <p className="mt-4 text-neutral-600">Loading caregivers...</p>
          </div>
        ) : caregivers.length === 0 ? (
          <div className="p-8 text-center text-neutral-500">No caregivers found</div>
        ) : (
            ...
            <table className="min-w-full divide-y divide-neutral-200">
              <thead className="bg-neutral-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-neutral-500 uppercase tracking-wider">Experience</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-neutral-500 uppercase tracking-wider">Employment</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-neutral-500 uppercase tracking-wider">Background</th>
              ...
              <tbody className="bg-white divide-y divide-neutral-200">
                {caregivers.map((caregiver) => (
                  <tr key={caregiver.id} className={`hover:bg-neutral-50 ${selectedIds.has(caregiver.id) ? 'bg-primary-50' : ''}`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <div className="ml-4">
                          ...
                          <div className="text-sm text-neutral-500">ID: {caregiver.id.slice(0, 8)}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-neutral-900">{caregiver.user.email}</div>
                      {caregiver.user.phone && <div className="text-sm text-neutral-500">{caregiver.user.phone}</div>}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-neutral-900">
                        {caregiver.yearsExperience ? `${caregiver.yearsExperience} years` : 'N/A'}
                      </div>
                      {caregiver.hourlyRate && <div className="text-sm text-neutral-500">${caregiver.hourlyRate}/hr</div>}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-neutral-900">{caregiver.employmentType?.replace('_', ' ') || 'N/A'}</div>
