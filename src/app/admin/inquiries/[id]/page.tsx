...
export default function AdminInquiryDetailPage() {
    ...
    return (
      <div className="min-h-screen bg-neutral-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-neutral-500">Loading inquiry...</p>
        </div>
      </div>
    );
  }
  ...
  if (error || !inquiry) {
    return (
      <div className="min-h-screen bg-neutral-50 flex items-center justify-center">
        <div className="text-center">
          <FiAlertTriangle className="w-12 h-12 text-error-500 mx-auto" />
          <p className="mt-4 text-error-600">{error || 'Inquiry not found'}</p>
          <Link href="/admin/inquiries" className="mt-4 inline-block text-primary-600 hover:underline">
            ← Back to Inquiries
          </Link>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Link href="/admin/inquiries" className="text-neutral-500 hover:text-neutral-700">
              ...
              <div>
                ...
                <p className="text-sm text-neutral-500">ID: {inquiry.id}</p>
              </div>
            </div>
                  ...
                  {[
                    ...
                    >
                      ...
                      {tab.count !== undefined && tab.count > 0 && (
                        <span className="ml-1 bg-neutral-100 text-neutral-600 px-2 py-0.5 rounded-full text-xs">
                          {tab.count}
                        </span>
                      )}
                    </button>
              ...
              <div className="p-6">
                {activeTab === 'details' && (
                  <div className="space-y-6">
                    {/* Status Edit */}
                    <div className="flex items-center justify-between">
                      <h3 className="text-lg font-semibold text-neutral-900">Inquiry Details</h3>
                      {editing ? (
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => setEditing(false)}
                            className="px-3 py-1.5 text-neutral-600 hover:text-neutral-800"
                          >
                        ...
                        <button
                          ...
                          className="flex items-center gap-2 px-3 py-1.5 text-primary-600 hover:bg-primary-50 rounded-lg"
                        >
                    ...
                    {editing && (
                      <div className="bg-primary-50 border border-primary-200 rounded-lg p-4 grid grid-cols-2 gap-4">
                        <div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-sm text-neutral-500">Contact Name</label>
                    ...
                    {inquiry.careRecipientName && (
                      <div className="border-t border-neutral-200 pt-4">
                        <h4 className="font-medium text-neutral-900 mb-3">Care Recipient</h4>
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                        ...
                        {inquiry.careNeeds && inquiry.careNeeds.length > 0 && (
                          <div className="mt-3">
                            <label className="text-sm text-neutral-500">Care Needs</label>
                            <div className="flex flex-wrap gap-2 mt-1">
                              {inquiry.careNeeds.map((need, i) => (
                                <span key={i} className="px-2 py-1 bg-neutral-100 rounded-full text-sm">{need}</span>
                              ))}
                            </div>
                          </div>
                        )}
                    ...
                    {inquiry.message && (
                      <div className="border-t border-neutral-200 pt-4">
                        ...
                        <p className="mt-1 text-neutral-700 whitespace-pre-wrap">{inquiry.message}</p>
                      </div>
                    )}
                    ...
                    <div className="border-t border-neutral-200 pt-4">
                      <div className="flex items-center justify-between mb-3">
                        <label className="font-medium text-neutral-900">Internal Notes</label>
                        <button
                      ...
                      {inquiry.internalNotes ? (
                        <div className="bg-warning-50 border border-warning-200 rounded-lg p-4 whitespace-pre-wrap text-sm">
                          {inquiry.internalNotes}
                        </div>
                      ) : (
                        <p className="text-neutral-400 italic">No notes yet</p>
                      )}
                    </div>
                ...
                {activeTab === 'responses' && (
                  <div className="space-y-4">
                    {inquiry.responses.length === 0 ? (
                      <p className="text-neutral-500 text-center py-8">No responses yet</p>
                    ) : (
                          ...
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                              <span className={`px-2 py-0.5 rounded text-xs ${
                                response.type === 'AI_GENERATED' ? 'bg-secondary-100 text-secondary-700' : 'bg-primary-100 text-primary-700'
                              }`}>
                                {response.type}
                              </span>
                              <span className="text-sm text-neutral-500">{response.channel}</span>
                            </div>
                            <span className={`px-2 py-0.5 rounded text-xs ${
                              response.status === 'SENT' ? 'bg-success-100 text-success-700' : 'bg-neutral-100 text-neutral-600'
                            }`}>
                              {response.status}
                            </span>
                          </div>
                          <p className="text-sm text-neutral-700 whitespace-pre-wrap">{response.content}</p>
                ...
                {activeTab === 'documents' && (
                  <div className="space-y-4">
                    {inquiry.documents.length === 0 ? (
                      <p className="text-neutral-500 text-center py-8">No documents uploaded</p>
                    ) : (
                      inquiry.documents.map(doc => (
                        <div key={doc.id} className="flex items-center justify-between border border-neutral-200 rounded-lg p-4">
                          <div className="flex items-center gap-3">
                            <FiFileText className="w-8 h-8 text-neutral-400" />
                            <div>
                              <p className="font-medium">{doc.fileName}</p>
                              <p className="text-sm text-neutral-500">{doc.documentType} • {doc.fileType}</p>
                ...
                {activeTab === 'followups' && (
                  <div className="space-y-4">
                    {inquiry.followUps.length === 0 ? (
                      <p className="text-neutral-500 text-center py-8">No follow-ups scheduled</p>
                    ) : (
                      inquiry.followUps.map(followup => (
                        <div key={followup.id} className="flex items-center justify-between border border-neutral-200 rounded-lg p-4">
                          <div className="flex items-center gap-3">
                            <FiClock className={`w-5 h-5 ${followup.status === 'COMPLETED' ? 'text-success-500' : 'text-warning-500'}`} />
                            <div>
                              <p className="font-medium">{followup.type}</p>
                              <p className="text-sm text-neutral-500">
                                Scheduled: {format(new Date(followup.scheduledFor), 'MMM d, yyyy h:mm a')}
                              </p>
                              {followup.notes && <p className="text-sm text-neutral-400 mt-1">{followup.notes}</p>}
                            </div>
                          </div>
                          <span className={`px-2 py-1 rounded text-xs ${
                            followup.status === 'COMPLETED' ? 'bg-success-100 text-success-700' :
                            followup.status === 'PENDING' ? 'bg-warning-100 text-warning-700' : 'bg-neutral-100 text-neutral-600'
                          }`}>
                            {followup.status}
                          </span>
                        </div>
                ...
                {activeTab === 'activity' && (
                  <div className="space-y-4">
                    {auditLogs.length === 0 ? (
                      <p className="text-neutral-500 text-center py-8">No activity recorded</p>
                    ) : (
                      auditLogs.map(log => (
                        <div key={log.id} className="flex items-start gap-3 border-l-2 border-neutral-200 pl-4 pb-4">
                          <div className="flex-1">
                            ...
                            {log.details && Object.keys(log.details).length > 0 && (
                              <pre className="mt-1 text-xs text-neutral-400 bg-neutral-50 p-2 rounded overflow-auto">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg border border-neutral-200 p-4">
              ...
              {inquiry.assignedTo ? (
                <div>
                  ...
                  <p className="text-sm text-neutral-500">{inquiry.assignedTo.email}</p>
                </div>
              ) : (
                <p className="text-neutral-400 italic">Unassigned</p>
              )}
            </div>
            ...
            <div className="bg-white rounded-lg border border-neutral-200 p-4">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div>
                  <label className="text-neutral-500">Last Updated</label>
                  <p className="font-medium">{format(new Date(inquiry.updatedAt), 'MMM d, yyyy h:mm a')}</p>
                </div>
                {inquiry.tourDate && (
                  <div>
                    <label className="text-neutral-500">Tour Date</label>
                    <p className="font-medium text-secondary-600">{format(new Date(inquiry.tourDate), 'MMM d, yyyy h:mm a')}</p>
                  </div>
                )}
              </div>
            </div>
