...
export default function PayoutsSettings() {
  ...
  useEffect(() => {
    if (status === "unauthenticated") {
      router.push("/auth/login?callbackUrl=/settings/payouts");
      return;
    }
  ...
  const fetchConnectStatus = async () => {
    try {
      ...
      const res = await fetch("/api/caregiver/payouts/connect/status", {
        method: "GET",
        headers: {
  ...
  const handleStartOnboarding = async () => {
    try {
      ...
      const res = await fetch("/api/caregiver/payouts/connect/start", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handlePayoutRequest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/caregiver/payouts/request", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      <div className="overflow-hidden rounded-lg bg-white shadow">
        ...
        <div className="border-b border-neutral-200 bg-neutral-50 px-4 py-5 sm:px-6">
          ...
          <p className="mt-1 text-sm text-neutral-500">
            Connect your bank account to receive payouts for your caregiving services.
          </p>
        </div>
