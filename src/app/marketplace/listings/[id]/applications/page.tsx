          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
            <div className="bg-neutral-50 p-4 rounded-md">
              <div className="flex items-center">
                <FiUsers className="text-neutral-400 mr-2" />
                <span className="text-sm font-medium text-neutral-500">Total</span>
              </div>
              <div className="mt-1 text-2xl font-semibold text-neutral-900">{applications.length}</div>
            </div>
            
            <div className="bg-primary-50 p-4 rounded-md">
              <div className="flex items-center">
                <FiClock className="text-primary-400 mr-2" />
                <span className="text-sm font-medium text-primary-500">New</span>
              </div>
              <div className="mt-1 text-2xl font-semibold text-primary-900">{statusCounts['APPLIED'] || 0}</div>
            </div>
            
            <div className="bg-success-50 p-4 rounded-md">
              <div className="flex items-center">
                <FiCheckCircle className="text-success-400 mr-2" />
                <span className="text-sm font-medium text-success-500">Accepted</span>
              </div>
              <div className="mt-1 text-2xl font-semibold text-success-900">{statusCounts['ACCEPTED'] || 0}</div>
            </div>
            
            <div className="bg-error-50 p-4 rounded-md">
              <div className="flex items-center">
                <FiXCircle className="text-error-400 mr-2" />
                <span className="text-sm font-medium text-error-500">Rejected</span>
              </div>
              <div className="mt-1 text-2xl font-semibold text-error-900">{statusCounts['REJECTED'] || 0}</div>
            </div>
          </div>
        </div>
        ...
        <div className="p-6">
          {applications.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-neutral-500 text-lg">No applications have been submitted yet.</p>
            </div>
          ) : (
              ...
              {applications.map((application: any) => (
                ...
                >
                  <div className="p-4 bg-neutral-50 border-b border-neutral-200 flex flex-wrap items-center justify-between gap-2">
                    <div className="flex items-center">
                      ...
                      <div className="ml-3">
                        ...
                        <p className="text-xs text-neutral-500">{application.caregiver.user.email}</p>
                      </div>
                    </div>
                  ...
                  <div className="p-4">
                    {application.note ? (
                      <div className="mb-3">
                        <h4 className="text-xs font-medium text-neutral-500 uppercase tracking-wider mb-1">Note</h4>
                        <p className="text-neutral-700 text-sm whitespace-pre-wrap line-clamp-3">{application.note}</p>
                      </div>
                    ) : null}
