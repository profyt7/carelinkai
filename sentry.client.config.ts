...
if (typeof window !== 'undefined') {
  console.log('[Sentry Client] DSN available:', !!SENTRY_DSN, SENTRY_DSN ? `(${SENTRY_DSN.substring(0, 30)}...)` : '(empty)');
}
...
if (SENTRY_DSN && SENTRY_DSN.startsWith('https://')) {
  ...
  console.log('[Sentry Client]   - Logs: enabled');
} else {
  console.log('[Sentry Client] ⚠️ DSN not configured - error tracking disabled');
}
