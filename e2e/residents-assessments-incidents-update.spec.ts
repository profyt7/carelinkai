...
test('edit assessment and incident inline', async ({ page, request }) => {
  ...
  // Poll the API to ensure the update has been persisted server-side to avoid UI race conditions
  await page.evaluate(async (rid) => {
    const sleep = (ms: number) => new Promise(res => setTimeout(res, ms));
    for (let i = 0; i < 25; i++) {
      ...
      if (r.ok) {
        const j = await r.json();
        const ok = (j.items || []).some((it: any) => it.type === 'Medication Error' && (it.severity || '').toUpperCase() === 'HIGH');
        if (ok) break;
      }
      await sleep(200);
    }
