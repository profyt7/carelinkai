...
// ── Post-signup verify-email UX (regression guard) ────────────────────────────
//
// An account is created PENDING and CANNOT sign in until the emailed link is
// clicked (src/lib/auth.ts gates status === 'PENDING'). The signup must therefore
// land the user on a clear "verify your email" state — NOT a bare login page and
// NOT a dashboard. This exact redirect has regressed twice (#675 fixed one of two
// register-page redirects; #681 fixed the catch fallback + removed the dead
// auto-signin), so it gets a dedicated regression test.

test.describe('@critical Register — post-signup verify-email state', () => {
  test('after signup the user lands on /auth/login?verify=1 with the verification-sent message + resend control', async ({
    page,
  }) => {
    const email = `reg-verify-${Date.now()}@test.carelinkai.com`;
    ...
    await expect(
      page.getByText(/we've sent a verification link to/i)
