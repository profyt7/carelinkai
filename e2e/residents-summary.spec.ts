...
test('resident summary PDF endpoint returns a PDF', async ({ page, request }) => {
  ...
  const residentId = await page.evaluate(async (args) => {
    const r = await fetch(`${location.origin}/api/residents`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify({ familyId: args.familyId, firstName: 'PDF', lastName: 'Check', dateOfBirth: '1945-05-05', gender: 'OTHER', status: 'ACTIVE' }) });
