...
### 2026-08-13 — Restore green main: e2e merge gate flaking on the committed-`.env` Sentry DSN (OL-121)
- **Objective:** The required **E2E (Residents + Family split)** check (`.github/workflows/e2e-family.yml`) was intermittently red on `main`, blocking deploys. Reproduce, root-cause, fix without weakening the suite (Risk-7 gate).
- **Diagnosis (evidence-driven, not the task's stated premise):** The nightly failures (08-01, 08-02, 08-04) were all on the SAME head `e981d43` → a **flake, not a code regression**. The residents/family/summary specs pass reliably (verified locally: 4/4 green). The real failures were in OTHER jobs of the same workflow:
  - **08-01, 08-02 = `e2e-concierge`.** The committed, tracked **`.env`** (not gitignored) carries a **live `SENTRY_DSN`/`NEXT_PUBLIC_SENTRY_DSN`** (`o4510740433076224.ingest.us.sentry.io`). CI's `npm run build` bakes it in → Sentry inits and phones home; in CI's locked-down network those calls hang — logs show `Failed to proxy …sentry.io… ETIMEDOUT`, `uncaughtException [Error: aborted] ECONNRESET`, then Next's `⚠ Server is approaching the used memory threshold, restarting…` mid-test → `page.evaluate: TypeError: Failed to fetch` → the `@critical` concierge flow times out.
  - **08-04 = `e2e-auth-signup-verify`.** Pure infra: `docker pull postgres:15` service container hit Docker Hub `context deadline exceeded` (3 retries). Not code.
- **Decision (spec-vs-code):** Neither the specs nor the product are wrong — the failure is **CI-harness flakiness**. Fix the harness, do NOT touch/skip/quarantine any spec.
- **Work completed:** Added `SENTRY_DSN: ""` + `NEXT_PUBLIC_SENTRY_DSN: ""` to the `env:` of all five e2e jobs. Empty (defined) process-env overrides the committed `.env` (Next `@next/env` only fills undefined keys) → build inlines an empty client DSN and `sentry.server/edge.config.ts` see a falsy DSN → **Sentry disabled** at runtime (verified: `[Sentry Server] ⚠️ No DSN provided, Sentry disabled`). Considered + **rejected** switching client-heavy jobs to `npm run start` — the standalone server doesn't serve `.next/static`, so the `'use client'` concierge/admin pages wouldn't hydrate (would turn flaky-red into solid-red).
- **Files changed:** `.github/workflows/e2e-family.yml` (only; +23 lines), `context/CARELINKAI_TECH_OPEN_LOOPS.md` (OL-121, OL-122), `context/CARELINKAI_TECHNICAL_STATE.md`, this file.
- **Commands run:** local Postgres 16 + `prisma migrate deploy`; `npm run build` (Sentry disabled → Compiled successfully); `playwright test --config=playwright.e2e.config.ts` on the full CI-active gate with `CI=true` + empty Sentry envs.
- **Tests/build status:** ✅ Build green. Gate run: **5 passed, 3 skipped** (OL-076-quarantined residents specs), concierge **flaky→green on retry** (cold `npm run dev` route-compile timeout on attempt 1 — a local dev artifact absorbed by CI `retries: 2`, NOT the Sentry failure), exit 0.
- **Deployment impact:** None — CI-only workflow env change. No schema/migration, no app code, no prod behavior change.
- **New risks/blockers:** **OL-122** — the committed `.env` holds LIVE secrets (Cloudinary secret, Resend key, Abacus key, NextAuth secret, Sentry/Bugsnag). Needs founder rotation + move to CI/Render secrets + gitignore + history scrub. Out of scope here (rotation + history rewrite is a founder call; untracking `.env` could change the CI build). Docker Hub pull flake (08-04) is transient infra, not code-fixable in-repo.
- **Recommended next step:** Merge the PR; confirm the next `e2e-family.yml` run on `main` is green. Then tackle OL-122 (secret rotation) and, separately, OL-076 (async-`params` migration that would let the 9 quarantined residents specs un-skip in CI and would quiet the concierge `params.id` warnings).

### 2026-09-15 — Self-host the founder intro video (remove HeyGen dependency)
- **Objective:** The DP follow-up emails (Touch 1 + Touch 3, "Watch the … intro") pointed at a HeyGen public link. HeyGen was canceled 9/15 (Creator → Free on 9/24), so the link could go dead/paywalled under a hospital planner. Replace it with a self-hosted page before 9/24. Do NOT touch `DP_FOLLOWUP_ENABLED` or sequence timing (not touched).
- **Work completed:**
  - New unauthenticated, chrome-free page **`/founder`** (`src/app/founder/page.tsx`): "CareLinkAI" wordmark, H1 "A 40-second intro from our founder", `<video controls playsInline preload="metadata" poster="/founder-intro-poster.jpg" src="/founder-intro.mp4">`, one contact line (reply to the email / `placements@getcarelinkai.com`). No forms, no extra tracking, `robots: { index: false, follow: false }`, mobile-first, max-width 720px centered.
  - **Middleware** (`src/middleware.ts`): `/founder` added to both always-public path lists; `/founder-intro.mp4` + `/founder-intro-poster.jpg` added to `PUBLIC_PATHS` AND excluded from the matcher so the video bytes (range requests) never hit the auth gate. Verified locally: `/founder` → 200 with no session, assets → 404 (file not yet present) not 307, `/settings` still → 307 login.
  - **Copy swap** (`src/lib/dp-outreach/copy.ts`): anchor text `VIDEO_LINK_TEXT` → "Watch the 40-second intro"; the two "90-second" body mentions (Touch 1, Touch 2) → "40-second" (video is 39s). `founderVideoUrl()` default (`dp-followup.ts`) → `https://getcarelinkai.com/founder` (exported as `DEFAULT_FOUNDER_VIDEO_URL`); `FOUNDER_VIDEO_URL` env override unchanged. `.env.example` + `docs/DP_LEAD_CAPTURE_AND_FOLLOWUP.md` updated. Repo grep: zero remaining `heygen` / `90-second` references (Education Hub, PRODUCT_DEMO, README had none).
  - **Tests:** updated `__tests__/dp-followup.test.ts` + `__tests__/dp-email-encoding.test.ts` (new default URL, "40-second" anchor, explicit no-`heygen`/no-`90-second` guards across all 4 touches). New jsdom render test `__tests__/founder-page.render.test.tsx` (video src/poster/controls/playsInline, H1, mailto, no forms, noindex metadata). New e2e `e2e/founder-page.spec.ts` (raw GET `/founder` → 200 + `<video src="/founder-intro.mp4">` + noindex meta; rendered page not redirected to login; asset paths not auth-gated) — wired into the `e2e-concierge` CI job.
- **⚠️ NOT done — needs Chris (binary not reachable from the agent env):** the MP4 is only on Chris's machine (HeyGen download `founder.mp4`, 39s/1080p). Not in the repo, Google Drive, or any remote branch; the HeyGen link is login-gated. Before merge, add:
  - `public/founder-intro.mp4` — if > 50 MB: `ffmpeg -i ~/Downloads/founder.mp4 -vcodec libx264 -crf 24 -preset slow -acodec aac -b:a 128k -movflags +faststart public/founder-intro.mp4` (target ~10–20 MB; `+faststart` matters for mobile Safari).
  - `public/founder-intro-poster.jpg` — `ffmpeg -ss 00:00:02 -i public/founder-intro.mp4 -frames:v 1 public/founder-intro-poster.jpg`.
  Until both exist, `/founder` renders but the player shows a black box (assets 404). Logged as **OL-123**.
- **Files changed:** `src/app/founder/page.tsx` (new), `src/middleware.ts`, `src/lib/dp-outreach/copy.ts`, `src/lib/dp-outreach/dp-followup.ts`, `.env.example`, `docs/DP_LEAD_CAPTURE_AND_FOLLOWUP.md`, `__tests__/dp-followup.test.ts`, `__tests__/dp-email-encoding.test.ts`, `__tests__/founder-page.render.test.tsx` (new), `e2e/founder-page.spec.ts` (new), `.github/workflows/e2e-family.yml`, context files.
- **Commands run:** `npm ci`, `npx prisma generate`, `npx jest` (3 suites: 37/37), `npx eslint` on changed files (clean), `npm run type-check` (clean), `next dev` + curl verification, `playwright test e2e/founder-page.spec.ts` (2/2 green with the sandbox's local Chromium).
- **Tests/build status:** ✅ jest 37/37 · eslint clean · tsc clean · e2e 2/2.
- **Deployment impact:** No schema/migration, no env changes required (`FOUNDER_VIDEO_URL` override still works). Once merged, every NEW Touch 1/3 send links to `/founder`. Already-sent emails still carry the HeyGen URL — nothing to do about those. Adds a ~10–20 MB static file to the Docker image (`public/` is copied into the runner stage).
