...
- **Provider Marketplace Listing ($99/mo):** Stripe Checkout + Customer Portal at `/settings/provider/billing`. Webhook syncs `listingStatus`. Providers with CANCELED/PAST_DUE/INCOMPLETE status hidden from marketplace. Null = grace period (visible). Requires `STRIPE_PRICE_PROVIDER_LISTING` env var.
- **Pro Caregiver ($19/mo):** Stripe Checkout + Customer Portal at `/settings/billing`. Webhook sets `isPro=true` on ACTIVE/TRIALING. Pro caregivers ranked first in all sort orders (`isPro: desc` in orderBy). ★ Pro badge on CaregiverCard. `applicationCount` tracked (enforcement not yet built). Requires `STRIPE_PRICE_PRO_CAREGIVER` env var.
...
2. **Raise placement fee** — Update `PLACEMENT_FEE_CENTS` from `50000` to `150000` in Render dashboard (OL-030).
3. **Build application cap enforcement** — Increment `applicationCount` on caregiver job application, block when over limit, monthly reset cron.
