...
## Active Branch
`main` — **2026-06-23 OL-083 publish-wide rollout merged (6 PRs): #588 (soften compliance claims → "HIPAA-aligned"), #589 (anonymous inquiry capture — `Inquiry.familyId` nullable, migration `20260623000001_inquiry_nullable_family`), #590 (unclaimed-listing search badge), #591 (`pre-publish-test-demo-sweep.ts`), #592 (`publish-directory-homes.ts` quality-gated DRAFT→ACTIVE publisher), #593 (branded photo placeholder + add-photos nudge); #595 (OL-083 Part B — `seed-cleveland-metro.ts`, the Cowork 6-county RCF master list seeded as 168 DRAFT rows, SNF-primary gated).** The seed (#595), purge (#591), and publish (#592) scripts are all **dry-run-default and await the founder's run/`--force` on Render — no prod data mutated yet** (founder confirmed both publish-phase dry-runs 2026-06-23: 16 publishable, 0 purge-eligible). Session-wrap on `docs/session-wrap-publish-epic` (fresh off main). Earlier: #578 (OL-079 instant claim email), #579 (batch-2 seed + enrich runbook), #580 (batch-2 cleanup script). ⚠️ `claude/inspiring-mayer-rvgyys` is a stale graveyard branch — do not push onto it.
