...
## Active Branch
`main` — **2026-06-23 OL-083 CLOSED: Greater-Cleveland directory is LIVE (128 public ACTIVE listings, 6 counties).** Publish-wide rollout (#588–#593) + Part B metro seed (#595) + enrich-reach (#597 `--include-unpopulated`) + address-anchor & city-match guard (#598) + cross-match cleanup (#599 `hold-crossmatch-homes.ts`) all merged. **Executed on Render prod 2026-06-23:** seed 118 metro homes (+138 city/OH anchors) → Places address backfill (117 in-state) → `publish-directory-homes.ts --force` published 133 → `hold-crossmatch-homes.ts --force` reverted 5 same-city cross-matches → **128 live**. $0 Anthropic spend. Migration on main: `20260623000001_inquiry_nullable_family` (anonymous capture). Earlier: #578 (OL-079 instant claim email), #579/#580 (batch-2 seed/cleanup). ⚠️ `claude/inspiring-mayer-rvgyys` is a stale graveyard branch — do not push onto it.
