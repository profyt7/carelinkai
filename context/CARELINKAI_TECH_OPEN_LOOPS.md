...
- **Status:** 🟡 CODE DONE, ASSETS PENDING — 2026-09-15 (branch `claude/relaxed-hamilton-lv0qe6`). HeyGen subscription canceled 9/15; the public link (`app.heygen.com/videos/founder-…`) the DP Touch 1/3 emails pointed at may go dead or paywalled on 9/24.
...
- **What's missing (founder action — the binary lives only on Chris's machine):** commit `public/founder-intro.mp4` (transcode if > 50 MB: `ffmpeg -i founder.mp4 -vcodec libx264 -crf 24 -preset slow -acodec aac -b:a 128k -movflags +faststart public/founder-intro.mp4`) and `public/founder-intro-poster.jpg` (`ffmpeg -ss 00:00:02 -i public/founder-intro.mp4 -frames:v 1 public/founder-intro-poster.jpg`). Until then `/founder` renders with a black player (assets 404 — not auth-redirected).
