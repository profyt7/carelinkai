...
- **Re-enable checklist (only after review):** (1) narrow recipients via `stop-drip.ts` + verify address quality; (2) wire the Resend webhook (`RESEND_WEBHOOK_SECRET`) + confirm suppression populates; (3) merge the C copy; (4) set `CLAIM_DRIP_ENABLED=1` in Render, re-add the `schedule:` lines (revert #687), re-enable the workflow in the Actions UI.
