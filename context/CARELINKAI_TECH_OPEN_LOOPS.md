...
- **Fixes for review (PR #688, NOT merged — drip stays paused; even if merged, `CLAIM_DRIP_ENABLED` defaults OFF):**
  ...
  - **C — copy rewrite (implemented in #688 for review):** cadence cut 4→**2 touches**; named-human sender (Chris, chris@ via OUTREACH_REPLY_TO); touch-1 references the real family event (+ waiting count only when >1), touch-2 is a neutral soft follow-up (no false "a family asked"); easy unsubscribe kept.
