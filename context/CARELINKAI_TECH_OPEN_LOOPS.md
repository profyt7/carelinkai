...
### OL-109: Claim drip PAUSED (autonomous outreach collided with humans + ~40% bounce)
- **Status:** 🔴 PAUSED 2026-07-01 — drip is OFF at three layers. Re-enable is gated on the fixes below + explicit founder review. Do NOT re-enable until reviewed.
- **What happened:** the daily claim-drip cron (`claim-drip.yml`, OL-100/#658) was autonomously emailing directory facilities (~40% bounce), colliding with human outreach (VA calls + personal 1:1 claim emails to warm leads — Pleasant Pointe, Eliza Jennings, Maplewood, …) and burning getcarelinkai.com sender reputation.
- **Pause (LIVE, three layers):**
  1. Founder **disabled the GitHub Actions workflow** in the UI (instant; stops the cron / touches 2-4).
  2. **#687** removed the `schedule:` trigger from `claim-drip.yml` (durable; keeps `workflow_dispatch`; deletes no code).
  3. **#689 — master `CLAIM_DRIP_ENABLED` (default OFF)** in `claim-drip.ts` gates BOTH `advanceClaimDrips` (cron) AND `startClaimDripOnLead` (event-driven touch-1 on inquiries/tours — the gap the cron pause didn't cover). A redeploy can never silently resume outreach.
- **Fixes for review (PR #688, NOT merged — drip stays paused; even if merged, `CLAIM_DRIP_ENABLED` defaults OFF):**
  - **A — recipient narrowing:** `scripts/stop-drip.ts` sets `claimDripStoppedReason='manual'` by `--home-id`/`--email`/`--name+--city`/`--file` (dry-run default; `--file` accepts `Name | City` lines; engine permanently skips + won't restart 'manual' homes). Founder's 23-facility VA/personal/Eliza exclusion list to be run in the Render shell (validated resolver provided).
  - **B — bounce + validation:** new `POST /api/webhooks/resend` (Svix-verified, fail-closed) → hard bounces/complaints into `EmailSuppression` (engine already consults it pre-touch); `emailLooksSendable()` rejects placeholder/invalid `outreachEmail`s pre-send (marks `invalid_email`).
  - **C — copy rewrite (Chris-approved copy in #688 for review):** cadence cut 4→**2 touches**; named-human sender (Chris Tolliver, chris@getcarelinkai.com via OUTREACH_REPLY_TO) + reply-to monitored; **PROACTIVE honest demand framing that NEVER fabricates a specific family inquiry** — touch-1 "families are searching {City} for care like {Facility}" (A/B subject, split by recipient), touch-2 soft "still taking new residents?" nudge; CAN-SPAM footer (unsubscribe + postal) + phone (216) 245-9482 in the sign-off. `{FirstName}` falls back to "there" (drip has only `outreachEmail`, no operator name), `{City}` from `address.city` → "your area" fallback. Copy regression test `__tests__/claim-drip-copy.test.ts` asserts the no-fabricated-inquiry + named-sender + CAN-SPAM rules.
  - **RESERVED real-inquiry variant** (subject "A family asked about {Facility}") — gated behind `args.realInquiry`; the engine does NOT set it. **Product-verification (asked before enabling it): YES** — when an operator claims a home, pending inquiries surface immediately: `GET /api/operator/inquiries` filters by `where.home = { operatorId }` and the claim route reassigns `home.operatorId` to the claimer; inquiries are linked by `homeId` with no per-inquiry ownership snapshot, so pre-claim inquiries on the (formerly sentinel-owned) home appear at once. So the reserved variant is truthful and safe to enable at founder discretion — still off by default.
- **Decisions (2026-07-01, Chris):** first-name personalization **intentionally skipped** — unclaimed listings store no operator name, "Hi there," is fine/honest, and the named decision-makers (Teresa, Lisa, Anita's sheet) get personal 1:1s + are excluded from the drip anyway. Copy wording is **final/approved — no tweaks.** Reserved real-inquiry variant is **approved to use, but wire it at re-enable time** (not now — drip is gated OFF).
- **Re-enable checklist (only after review):** (1) narrow recipients via `stop-drip.ts` + verify address quality; (2) wire the Resend webhook (`RESEND_WEBHOOK_SECRET`) + confirm suppression populates; (3) merge #688 (copy already approved); (3b) **wire the reserved real-inquiry variant** — in the engine's touch-1 path, pass `realInquiry: true` ONLY for homes with a genuine pending inquiry (a `NEW` `Inquiry` on that `homeId`); proactive copy for all others; (4) set `CLAIM_DRIP_ENABLED=1` in Render, re-add the `schedule:` lines (revert #687), re-enable the workflow in the Actions UI.
- **Done when:** #688 approved + merged and the founder deliberately re-enables via the checklist. Until then, paused.
