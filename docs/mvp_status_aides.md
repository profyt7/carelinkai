...
| Aide skills & tags       | Aide      | Add skills, certifications, experience              | DONE   | End-to-end implemented: DB fields on Caregiver (specialties, settings, careTypes) in prisma/schema.prisma; API: src/app/api/profile/route.ts validates against MarketplaceCategory and persists arrays; UI: src/app/settings/profile/page.tsx renders "Skills & Tags" with toggles for Specialties/Settings/Care Types; Marketplace integration: filters in src/app/marketplace/page.tsx and server filtering in src/app/api/marketplace/caregivers/route.ts use these facets. |
