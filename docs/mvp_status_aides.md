# CareLinkAI MVP Phase 1 Status Matrix (Aide / Caregiver Marketplace)
...
| Aide availability        | Aide      | Set/update availability (days/times)                | WIP    | AvailabilitySlot model present; availability checking API exists at /api/calendar/availability. No caregiver-facing UI to create/edit availability slots yet. |
...
| Aide filters             | Operator  | Filter aides by location, skills, availability      | WIP    | Location/specialty/settings filters work. Availability filtering supported in API (/api/marketplace/caregivers: availableOnDay/availableBetween/availableStart/availableEnd) but UI controls are not present on /marketplace yet. |
| Aide detail view         | Operator  | View aide profile, skills, docs, availability       | WIP    | Detail shows photo, bio, rate, experience, specialties, reviews, and request shift form. No availability calendar display and no credentials/docs listing. UI: /marketplace/caregivers/[id]. |
| Operator <-> Aide contact  | Operator  | Send an initial contact / message to aide           | WIP    | Messaging system exists (/api/messages, /messages). New message picker lists employed caregivers; marketplace "Message" button does not deep link to a specific caregiver thread. |
